...
export default function BoostCard({ name, boosts, description, AllBoosts }) {
    console.log({ AllBoosts });
            ...
            <ThemeProvider theme={theme}>
                <Paper elevation={3} sx={{ height: "100%" }}>
                    ...
                    <Grid container spacing={0} justifyContent="center">
                        {filteredBoosts.map((element, index) => <Grid item key={index}>{element?.image ? (<Image src={element?.image} width={75} height={75} alt="" />) : null}</Grid>)}
                    </Grid>
